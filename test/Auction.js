...
contract("Auction", function (accounts) {
  ...
  context("1. Auction Setup Validation Tests", async function () {
    it("NonNullToken - Requires a non-null token to initiate auction", async function () {
      await expectRevert(Auction.new(price, owner, ZERO_ADDRESS, tokenSupply), "Auction: token is the zero address");
    });
    ...
    it("NonZeroPrice - Requires a non-zero price for the auction.", async function () {
      await expectRevert(Auction.new(0, owner, this.token.address, tokenSupply), "Auction: price is 0");
    });

    it("NonNullOwner - Requires a non-null owner for the auction.", async function () {
      await expectRevert(Auction.new(price, ZERO_ADDRESS, this.token.address, tokenSupply), "Auction: owner is the zero address");
    });

    it("NonZeroTokenMaxAmount - Requires a non-zero tokenMaxAmount for the auction.", async function () {
      await expectRevert(Auction.new(price, owner, this.token.address, 0), "Auction: tokenMaxAmount is 0");
    });
  });
  context("2. Deployment and Getter Functions Tests", async function () {
    beforeEach(async function () {
      this.token = await SimpleToken.new(tokenSupply);
      this.auction = await Auction.new(price, owner, this.token.address, tokenSupply);
  ...
  context("5. Finalization Functionality Tests", async function () {
    ...
    it("ReturnAllowOwnerWithdraw - Should return the correct allow owner withdrawl status.", async function () {
      expect(await this.auction.allowOwnerWithdrawl()).to.be.false;
      await this.auction.finalize({ from: owner });
      expect(await this.auction.allowOwnerWithdrawl()).to.be.true;
    });
    ...
    it("AssignTokensToBeneficiary - Should assign tokens to beneficiary after finalization.", async function () {
      ...
      expect(await this.token.balanceOf(purchaser)).to.be.bignumber.equal(expectedTokenAmount);
    });
  ...
  context("6. Auction with Insufficient Token Balance Tests", async function () {
    beforeEach(async function () {
      this.token = await SimpleToken.new(tokenSupply);
      // Initialize the tested contract with the initialRate
      this.auction = await Auction.new(price, owner, this.token.address, insufficientTokenSupply);
